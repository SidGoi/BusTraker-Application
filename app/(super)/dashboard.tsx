...
import SuperLiveMap from "@/components/SuperLiveMap";
...
export default function SuperAdminDashboard() {
      ...
      if (json.success) {
        ...
        if (selectedBus) {
          const updated = json.data.find(
            (b: any) => b.busId === selectedBus.busId,
          );
  ...
  const getStatus = (lastUpdate?: string) => {
    ...
    const diff =
      (new Date().getTime() - new Date(lastUpdate).getTime()) / 60000;
    return { active: diff <= 3, label: diff <= 3 ? "ACTIVE" : "INACTIVE" };
  };

  const counts = useMemo(() => {
    const active = buses.filter((b) => getStatus(b.lastUpdate).active).length;
    return { active, inactive: buses.length - active };
  }, [buses]);
  ...
  const handleLogout = () => {
    Alert.alert(
      "Terminate Session",
      "Are you sure you want to exit Master Control?",
      [
  ...
  const handleSearch = (text: string) => {
    setSearchQuery(text);
    const found = buses.find((b) => b.busId.toString() === text.trim());
    ...
    <SafeAreaView className="flex-1 bg-slate-900">
      ...
      <View className="px-6 py-4 flex-row items-center justify-between border-b border-slate-800 bg-slate-900">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-black text-white ml-2 uppercase tracking-tighter">
            Master Control
          </Text>
        </View>
        <Pressable
          onPress={handleLogout} // FIXED: Now calling handleLogout
      ...
      <Pressable
        ...
        className="mx-6 mb-4 flex-row bg-slate-800/50 p-3 rounded-2xl border border-slate-700 justify-around items-center"
      >
        <View className="flex-row items-center">
          <View className="w-2 h-2 rounded-full bg-green-500 mr-2" />
          <Text className="text-white font-bold">
            {counts.active} Active Units
          </Text>
        </View>
        ...
        <View className="absolute top-4 left-6 right-6">
          <View className="bg-slate-900/90 p-3 rounded-2xl shadow-2xl border border-slate-700 flex-row items-center justify-between">
            <View className="flex-row items-center">
              ...
              <Text className="ml-2 font-bold text-white text-[11px] uppercase tracking-wider">
                Fleet Oversight
              </Text>
            </View>
            <View className="flex-row gap-2 items-center">
              {isRefreshing && (
                <RefreshCw size={10} color="#22c55e" className="mr-5" />
              )}
        ...
        {!!selectedBus && (
          <View className="absolute bottom-6 left-6 right-6 bg-white rounded-[32px] p-6 shadow-2xl border border-slate-100">
            <View className="flex-row justify-between items-start mb-4">
              <View>
                <Text className="text-2xl font-black text-slate-900">
                  Unit #{selectedBus.busId}
                </Text>
                <Text className="text-blue-600 font-bold uppercase tracking-widest text-[10px]">
                  {selectedBus.zone} Sector
                </Text>
              </View>
              <View
                className={`px-4 py-1.5 rounded-full ${getStatus(selectedBus.lastUpdate).active ? "bg-green-100" : "bg-slate-100"}`}
              >
                <Text
                  className={`text-[10px] font-black ${getStatus(selectedBus.lastUpdate).active ? "text-green-600" : "text-slate-400"}`}
                >
                  {getStatus(selectedBus.lastUpdate).label}
                </Text>
              </View>
            </View>
            ...
            <View className="flex-row gap-3">
              ...
              >
                <Text className="text-slate-600 font-bold">Dismiss</Text>
              </Pressable>
              <Pressable
                onPress={() =>
                  Linking.openURL("https://bus-attendance-sys.vercel.app/")
                }
                className="flex-row items-center justify-center flex-[1.5] bg-red-600 py-4 rounded-2xl shadow-lg active:opacity-80"
              >
                <ExternalLink size={18} color="white" />
                <Text className="text-white font-bold ml-2">Open Portal</Text>
              </Pressable>
            </View>
          </View>
        )}
      ...
      {!selectedBus && (
        <View className="bg-slate-900 px-6 pt-6 pb-8 border-t border-slate-800">
          <Pressable
            ...
            className="bg-blue-600 flex-row items-center justify-between px-6 py-5 rounded-[24px] shadow-lg active:opacity-90 shadow-blue-500/20"
          >
            <View className="flex-row items-center">
              <ExternalLink size={20} color="white" />
              <Text className="text-white text-lg font-black ml-3">
                Fleet Attendance Portal
              </Text>
            </View>
              ...
              renderItem={({ item }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View
                      className={`${active ? "bg-green-500/10 border-green-500/20" : "bg-slate-500/10 border-slate-500/20"} px-3 py-1 rounded-full border`}
                    >
                      <Text
                        className={`${active ? "text-green-500" : "text-slate-500"} font-bold text-[10px]`}
                      >
                        {label}
                      </Text>
                    </View>
